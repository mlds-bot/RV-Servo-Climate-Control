...
--------------------
The Diagnostics page now controls the onboard RGB LED without changing the
servo, thermostat, cooling-verification, Wi-Fi, or other controller behavior.

Global brightness range:
  0   = completely off
  255 = maximum brightness

Each status has its own red, green, blue, and blink interval setting:
  READY, FAN, COOL, COOL_VERIFIED, HEAT, PENDING, WARNING,
  SYNC, MOVING, WIFI, MQTT, DHT

Blink interval is in milliseconds. A value of 0 means solid.
Settings are saved in ESP32 Preferences and survive a reboot.

MQTT topics:
  rv/servo/led/command
  rv/servo/led/state

Commands:
  BRIGHTNESS,0-255
  PROFILE,STATUS,R,G,B,BLINK_MS
  TEST,R,G,B,BLINK_MS
  AUTO
  STATUS
